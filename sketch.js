...
class LeftPanel {
  constructor(h){
    ...
    this.links = [
      ...
      { label: "第一單元作品", url: "https://minawu20070717-ux.github.io/414730415/" },
      { label: "第一單元筆記", url: "https://hackmd.io/@GWwvuOp4TXSe8EZ8o9tX2w/ryKgFuynxx" },
      { label: "測驗卷", url: "http://127.0.0.1:5501/index.html" },
      { label: "作品筆記", url: "https://hackmd.io/@GWwvuOp4TXSe8EZ8o9tX2w/B1tXodygbg" },
    ...
    this.submenus = {};
    // 自動尋找「淡江大學」位置並加入子選單
    const tkuIdx = this.links.findIndex(l => l.label === "淡江大學");
    if (tkuIdx >= 0) {
      this.submenus[tkuIdx] = [
        { label: "教育科技學系", url: "https://www.et.tku.edu.tw/" }
      ];
    }

    this.linkRects = []; // 每幀繪製時更新，供點擊檢測使用（包含子選單時也會一起放入）
    this.linkActions = []; // 與 linkRects 平行，記錄每個 rect 的動作來源（main / submenu）
  ...
  // 面板被點擊時呼叫（若點到連結或子選單則開啟）
  onClickAt(mx, my){
    const idx = this.getLinkAt(mx, my);
    if (idx >= 0){
      const action = this.linkActions[idx];
      if (!action) return;
      if (action.type === 'main') {
        const link = this.links[action.idx];
        if (link.url === "home") {
          closeOverlay();
        } else {
          openOverlay(link.label, link.url);
        }
      } else if (action.type === 'submenu') {
        const parentIdx = action.parent;
        const subIdx = action.idx;
        const submenu = this.submenus[parentIdx][subIdx];
        // 與主選單相同的開啟方式：使用 overlay 顯示（若要改成新分頁，換回 window.open）
        openOverlay(submenu.label, submenu.url);
      }
    }
  }
  ...
  show(){
    ...
    // 若要讓「回到首頁」與上方子選單保留較大間距，可在此調整
    const homeIdx = this.links.findIndex(l => l.label === "回到首頁");
    const extraGap = 24; // 可調整為想要的距離（像素）

    for (let i = 0; i < this.links.length; i++){
        // 計算每一項的 y（若為或在回到首頁之後則加入額外間距）
        const baseY = this.topOffset + i * (this.textSize + this.lineSpacing);
        const ty = baseY + (homeIdx >= 0 && i >= homeIdx ? extraGap : 0);
        ...
        this.linkActions.push({ type: 'main', idx: i });
        ...
        // 若此主連結有子選單，判斷是否要顯示（當滑鼠在父項或在子選單區內時顯示）
        if (this.submenus[i]) {
          const subs = this.submenus[i];
          const submenuHeight = subs.length * (this.textSize + 6);
          // 檢查滑鼠是否在子選單將會出現的區域（包含子選單自身），或父項被 hover
          const willShowSub = isHovered ||
            (mouseX >= this.x && mouseX <= this.x + this.w &&
             mouseY >= rectY + rectH && mouseY <= rectY + rectH + submenuHeight);
          
          if (willShowSub) {
            // 畫出子選單背景（較淡）
            noStroke();
            fill(250, 250, 250, 220);
            rect(this.x, rectY + rectH, this.w, submenuHeight, 4);

            // 逐項繪製子選單項目，並加入 rect 與 action
            for (let j = 0; j < subs.length; j++) {
              const subY = rectY + rectH + j * (this.textSize + 6) + 4;
              // push rect/action
              this.linkRects.push({ x: this.x, y: subY, w: this.w, h: this.textSize + 6 });
              this.linkActions.push({ type: 'submenu', parent: i, idx: j });

              // 判斷子項是否被 hover（改變文字顏色）
              const subHovered = mouseX >= this.x && mouseX <= this.x + this.w &&
                                 mouseY >= subY && mouseY <= subY + (this.textSize + 6);

              if (subHovered) fill(255, 0, 0);
              else fill(40, 200);

              textAlign(LEFT, TOP);
              textSize(this.textSize - 2);
              if (this.x + this.w > 0) {
                text(subs[j].label, this.x + 14, subY);
              }
              // restore center alignment for following main links
              textAlign(CENTER, TOP);
              textSize(this.textSize);
            }
          }
...
function draw() {
    ...
    fill(255, 230);
    rectMode(CORNER);
    rect(width - boxW - 12, 12, boxW, boxH, 6);
    fill(20);
    text(infoText, width - 20, 16);
    pop();
  ...
  if (url === "about") {
    ...
    content.innerHTML = `
      <div style="position:relative;height:100%;">
        <div style="position:absolute;top:24px;right:24px;z-index:10;display:flex;flex-direction:column;align-items:flex-end;gap:8px;">
          <div style="background:rgba(255,255,255,0.95);padding:6px 10px;border-radius:6px;font-weight:600;color:#111;box-shadow:0 2px 6px rgba(0,0,0,0.12);text-align:right;">
            吳湘湘<br><span style="font-size:14px;opacity:0.85;">414730415</span>
          </div>
          <img src="./assets/mee.jpg" alt="個人照片" style="width:150px;height:150px;border-radius:8px;object-fit:cover;box-shadow:0 4px 8px rgba(0,0,0,0.2);" onerror="this.style.border='2px solid red';">
        </div>
        <div style="height:100%;overflow:auto;padding:24px;padding-right:220px;">
          <h2 style="margin-top:0;">自我介紹</h2>
          <p>我叫吳湘湘<br>來自淡江大學 教育科技學系<br>興趣是：吃美食</p>
        </div>
      </div>
    `;
